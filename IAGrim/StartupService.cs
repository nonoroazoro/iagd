...
namespace IAGrim {
    public class StartupService {
        ...
        public static bool PerformMissingExpansionDataCheck(
            ...
            try {
                ...
                foreach (var marker in ExpansionDataMarkers) {
                    ...
                    if (!AutoParseDatabase(parsingService, databaseItemDao, playerItemDao, settings, gdPath)) {
                        return false;
                    }

                    // Only a completed parse counts as handled. Transient read failures must be retried.
        ...
        /// </summary>
        private static bool AutoParseDatabase(
            ParsingService parsingService,
            IDatabaseItemDao databaseItemDao,
            IPlayerItemDao playerItemDao,
            SettingsService settings,
            ...
            if (!parsingService.Execute()) {
                Logger.Warn("Automatic database parse failed; preserving the previous item stats and settings.");
                return false;
            }
            ...
            settings.GetLocal().CurrentGrimdawnMod = modPath;
            ...
            settings.GetLocal().ParsedLanguageCode = parsingService.LanguageCode;
            ...
            RuntimeSettings.InitializeLanguage(
                settings.GetLocal().LanguageCode,
                settings.GetLocal().ParsedLanguageCode,
                databaseItemDao.GetTagDictionary());
